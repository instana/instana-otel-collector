...
MODS := $(shell go list -m all | awk '{print $$1}')
...
RECEIVER_MODS := $(shell go list -m all | grep 'receiver')
...
PROCESSOR_MODS := $(shell go list -m all | grep 'processor')
...
EXPORTER_MODS := $(shell go list -m all | grep 'exporter')
...
EXTENSION_MODS := $(shell go list -m all | grep 'extension')
CONNECTOR_MODS := $(shell go list -m all | grep 'connector')
INTERNAL_MODS := $(shell go list -m all | grep 'internal' || true)
PKG_MODS := $(shell go list -m all | grep 'pkg' || true)
CMD_MODS := $(shell go list -m all | grep 'cmd' || true)
...
otelcontribcol: genotelcontribcol
	cd ./cmd/otelcontribcol && GO111MODULE=on CGO_ENABLED=0 $(GOCMD) build -trimpath -o ../../bin/instana-otel-collector_$(GOOS)_$(GOARCH)$(EXTENSION) \
		-tags $(GO_BUILD_TAGS) .
