...
install_service() {
	...
	WRAPPER_SCRIPT="$SCRIPT_PATH/bin/instana_supervisor_wrapper.sh"
...
EOF
	...
	mkssys -s "$SUBSYSTEM_NAME" \
		...
		-o "$SCRIPT_PATH/logs/supervisor.out" \
		-e "$SCRIPT_PATH/logs/supervisor.err"
	...
	echo "Service $SERVICE_NAME (Supervisor) has been installed and started."
}
...
uninstall_service() {
	...
	rm -f "$SCRIPT_PATH/bin/instana_supervisor_wrapper.sh"
}
